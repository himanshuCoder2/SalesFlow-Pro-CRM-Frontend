...
const schedule = [
  ...
  },
  {
    time: "04:00",
    period: "PM",
    title: "Internal Sales Sync",
    company: "sales-team-huddle",
    ...
    <MobileShell>
      <header className="flex items-center p-6 justify-between">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="size-12 rounded-full border-2 border-primary/20 bg-primary/10 flex items-center justify-center text-primary font-bold">
              {(user.name || "U")
                .split(" ")
                .map((n) => n[0])
                .join("")
      ...
      <section className="px-6 mb-6">
        <div className="flex justify-between items-center mb-3">
          ...
          <Link href="/pipeline" className="text-primary text-xs font-bold">
            View Details
          </Link>
        </div>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
          {kpis.map((k) => (
            <div
              key={k.label}
              className="flex min-w-[140px] flex-col gap-2 rounded-xl p-4 bg-white border border-slate-200 shadow-sm"
            >
              ...
              <p
                className={`${k.up ? "text-green-600" : "text-red-500"
      ...
      <section className="px-6 mb-8">
        <h3 className="text-slate-900 text-sm font-bold uppercase tracking-wider mb-4">
          Quick Actions
        </h3>
        ...
        <div className="grid grid-cols-4 gap-3">
          {quickActions.map((a) => (
            ...
            >
              <div
                className={`size-14 flex items-center justify-center rounded-2xl ${a.filled
      ...
      <section className="px-6 pb-28">
        ...
        <div className="space-y-4">
          {schedule.map((s) => (
            <div
              key={s.title}
              className={`flex gap-4 p-4 rounded-xl bg-white border-l-4 ${s.color} shadow-sm ${s.dim ? "opacity-60" : ""
                }`}
            >
